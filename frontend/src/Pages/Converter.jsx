  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_MAIN_POINT_RENDER}/ai/get-converted-code`, {
        ...
        body: JSON.stringify({ code: data,lang: ConvertLanguage })
      });
      ...
      <div className="w-full lg:w-1/2 bg-purple-950 rounded-xl shadow-lg p-4 sm:p-6 flex flex-col gap-4 h-[calc(100vh-2rem)]">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <input
            ...
            placeholder="Enter current language "
            onChange={(e) => setSelectLanguage(e.target.value)}
            className="flex-1 p-3 bg-gray-700 border border-gray-600 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 text-white placeholder-gray-400 transition-all duration-200 placeholder:text-sm"
          />
          <button
          ...
          <input
            ...
            placeholder="Enter convert language "
            onChange={(e) => setConvertLanguage(e.target.value)}
            className="p-3 bg-gray-700 border border-gray-600 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 text-white placeholder-gray-400 transition-all duration-200 placeholder:text-sm"
          />
